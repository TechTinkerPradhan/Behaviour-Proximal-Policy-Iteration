...
if __name__ == "__main__":
    ...
    # epsilon
    clip_ratios = [0.05, 0.1, 0.2, 0.25, 0.3]
    
    # sigma
    clip_ratio_decays = [0.90, 0.94, 0.96, 0.98, 1.00]

    # adjust clip ratio
    for clip_ratio in clip_ratios:
        new_logger_path = os.path.join(logger_path, str(clip_ratio))
        logger = SummaryWriter(log_dir=new_logger_path, comment=comment)

        bppo = BehaviorProximalPolicyOptimization(device, state_dim, args.bppo_hidden_dim, args.bppo_depth, action_dim, args.bppo_lr, clip_ratio, args.entropy_weight, args.decay, args.omega, args.bppo_batch_size)
    ...
    # change decay rate
    for decay in clip_ratio_decays:
        new_logger_path = os.path.join(logger_path, str(decay))
        logger = SummaryWriter(log_dir=new_logger_path, comment=comment)

        bppo = BehaviorProximalPolicyOptimization(device, state_dim, args.bppo_hidden_dim, args.bppo_depth, action_dim, args.bppo_lr, args.clip_ratio, args.entropy_weight, decay, args.omega, args.bppo_batch_size)
